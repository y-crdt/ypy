...
use yrs::{Observable, Text, TextRef, TransactionMut};
...
#[pyclass(unsendable)]
#[derive(Clone)]
pub struct YText(pub SharedType<TextRef, String>);
...
impl YTextEvent {
    pub fn new(event: &TextEvent, txn: &TransactionMut) -> Self {
        YTextEvent {
            target: Python::with_gil(|py| YText::from(event.target().clone()).into_py(py)),
            delta: Python::with_gil(|py| {
                let delta = event.delta(txn).into_iter().map(|d| d.clone().into_py(py));
